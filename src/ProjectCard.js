...
import farming from "./project-thumbnails/farming.png";
...
function ProjectCard(props) {
	...
	const img =
		props.project.title === "IdleMMO"
			? idlemmo
			: props.project.title === "TypeFormer"
			? typeformer
			: props.project.title === "Typing Incremental"
			? typingincremental
			: props.project.title === "TypeRPG"
			? typerrpg
			: props.project.title === "Competative Typing"
			? competativetyping
			: props.project.title === "Bombdgeball"
			? bombdgeball
			: props.project.title === "Ooblies.com"
			? ooblies
			: props.project.title === "Idle Prospector"
			? idleprospector
            : props.project.title === "Farming Game"
            ? farming
			: defaultImage;
	...
	return (
		<div
			className={
				"bg-gray-200 rounded-xl grid grid-cols-2 overflow-hidden cursor-pointer border-2 " +
				borderStyle +
		...
		>
			...
			<div className="flex flex-col">
				<div
					className="font-bold pl-8 pr-8 pt-4 text-xl"
					onClick={toggleActive}
				>
					{props.project.title}
				</div>
				<div
					className="text-gray-600 flex-grow p-8 pt-4 max-h-40"
					onClick={toggleActive}
				>
				...
				<div
					className={
						"grid grid-flow-row p-8 text-gray-400 " + (active ? "" : " hidden")
					}
				>
					<div>
						...
						<br />
						Status:{" "}
						<span className={statusTextStyle}>{props.project.status}</span>
						<br />
				...
				<div
					className={
						"grid grid-flow-col text-center bg-gray-400" +
						(active ? "" : " hidden") +
						(props.project.sourceCode || props.project.playableLink
							? ""
							: " hidden")
					}
				>
					<div
						className={
							...
							(props.project.sourceCode ? "" : " hidden")
						}
					>
						<a
							className="inline-block h-full w-full p-4"
							rel="noreferrer"
							target="_blank"
					...
					<div
						className={
							...
							(props.project.playableLink ? "" : " hidden")
						}
					>
						<a
							className="inline-block h-full w-full p-4"
							rel="noreferrer"
							target="_blank"
							href={props.project.playableLink}
						>
							Play
						</a>
					</div>
